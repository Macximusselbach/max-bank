...
class ClientControllerTest extends TestBase {
    ...
    void shouldReturnOkStatusAndClient_whenGetById() throws Exception {
        when(clientService.findById(2L)).thenReturn(client2);

        mockMvc.perform(get("/client/2"))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(content().json("{'id':2,'name':'Teste2','email':'teste@gmail.com','cpf':'123456789','balance':0.0}"));

    }

    @Test
    void shouldReturnNotFoundStatus_whenGetById() throws Exception {
        when(clientService.findById(1L)).thenThrow(new RuntimeException());
    ...
    void shouldReturnCreatedStatusAndCreatedClient_whenPost() throws Exception {
        when(clientService.save(Mockito.any())).thenReturn(client1);
    ...
    void shouldReturnNotAcceptableStatus_whenPost() throws Exception {
        when(clientService.save(Mockito.any())).thenThrow(new RuntimeException());
    ...
    void shouldreturnOkStatusAndModifiedClient_whenPut() throws Exception {
        ...
        when(clientService.update(Mockito.any())).thenReturn(client1);
    ...
    void shouldReturnNotfoundStatus_whenPut() throws Exception {
        when(clientService.update(Mockito.any())).thenThrow(new RuntimeException());
